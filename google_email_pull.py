...
logging.basicConfig(filename='debugging/google_email_pull_debug.txt', level=logging.DEBUG)
...
def get_emails():
    ...
    def get_emails(result_bytes):
        ...
        for num in result_bytes[0].split():
            typ, data = imap_email.fetch(num, '(RFC822)')
            msgs.append(data)
        return msgs
    ...
    imap_email = imaplib.IMAP4_SSL(imap_url)
    imap_email.login(user, password)
    imap_email.select('Inbox')
    ...
    new_emails = get_emails_since(imap_email, since_date, before_date)
    ...
    for msg in msgs[::-1]:
        for sent in msg:
            if type(sent) is tuple:
                ...
                messege = email.message_from_bytes(content)
                sender = mail["From"]
                subject = mail["Subject"]
                date = mail["Date"]
                body = get_body(mail)
                clean_body = clean_string(str(body))
                
                for part in messege.walk():
