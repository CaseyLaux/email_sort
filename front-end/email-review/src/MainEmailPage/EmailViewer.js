...
// Redirect the user to the login page or landing page using the passed navigate function
navigate('/login');
}
async function checkLogin() {

const token = localStorage.getItem('jwt');
let load_url = `${subdomains.email_server}/api/v1/emailcheck`;
...
// This object only sends the completion value and the id to the endpoint

const payload = {
completion: completionValue,
_id: currentDetailViewEmail._id.$oid,
email_account: currentDetailViewEmail.email_address,
};
...
let move_url = `${subdomains.email_server}/api/update-email`;
...
// The everything box that holds everything inside of it
