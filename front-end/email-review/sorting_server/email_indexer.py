...
def index_emails():
    ...
    email_directory = os.path.abspath('../public/unsorted/')
    print(os.getcwd())
    ...
    email_files = [file for file in files if file.endswith('txt')]
...
def index_sorted_emails():
    # Set the email_directory to an absolute path
    email_directory = os.path.abspath('../public/sorted/')
    print(os.getcwd())
    # Make sure the directory exists
    if not os.path.exists(email_directory):
        print(f"The 'human_sorted' directory does not exist at {email_directory}")
